...
def prefetch_test(opt):
    ...
    for model_id in range(opt.model_begin, opt.model_end + 1):
        model_path = opt.save_dir[:-4] if opt.save_dir.endswith('TEST') else opt.save_dir
        if opt.load_model == '' or model_id > opt.model_begin:
        ...
        if opt.save_predictions:
            if opt.test_refine:
                save_json(output_hoi, model_path, 'predictions_model_' + str(model_id) + f"_refine" + '.json')
            elif opt.test_mul:
                save_json(output_hoi, model_path, 'predictions_model_' + str(model_id) + f"_mul" + '.json')
            elif opt.test_add:
                save_json(output_hoi, model_path, 'predictions_model_' + str(model_id) + f"_add" + '.json')
            else:
        ...
        if opt.test_with_eval:
            if 'hico' in opt.dataset:
                hoi_eval = hico(os.path.join(opt.root_path, 'hico_det/annotations/test_hico.json'),
                                os.path.join(model_path, 'model_' + str(model_id)))
