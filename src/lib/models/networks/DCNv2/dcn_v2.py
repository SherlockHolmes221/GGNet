...
class DCN_O1(DCNv2):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding,
                 dilation=1, deformable_groups=1):
        super(DCN_O1, self).__init__(in_channels, out_channels,
                                     kernel_size, stride, padding, dilation, deformable_groups)
        ...
        self.conv_offset_mask = nn.Conv2d(self.in_channels,
                                          ...
                                          bias=True)
        self.init_offset()
        self.refine_dcn_out = nn.ReLU(inplace=True)
    ...
    def forward(self, input1, input2):
        out = self.conv_offset_mask(input1)
        o1, o2, mask = torch.chunk(out, 3, dim=1)
        offset = torch.cat((o1, o2), dim=1)
        mask = torch.sigmoid(mask)
        func = DCNv2Function(self.stride, self.padding, self.dilation, self.deformable_groups)
        dcn_features = func(input2, offset, mask, self.weight, self.bias)
        dcn_features = self.refine_dcn_out(dcn_features)
        return offset, dcn_features


class DCN_CASC(DCNv2):

    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding,
                 dilation=1, deformable_groups=1):
        super(DCN_CASC, self).__init__(in_channels, out_channels,
                                       kernel_size, stride, padding, dilation, deformable_groups)
        ...
        self.init_offset()
        # todo
        self.refine_dcn_out = nn.Sequential(nn.ReLU(inplace=True))
        # self.refine_dcn_out = nn.Sequential(nn.ReLU(inplace=True),
    ...
    def forward(self, input1, input2, offset_1):
        out = self.conv_offset_mask(input1)
        o1, o2, mask = torch.chunk(out, 3, dim=1)
        offset = torch.cat((o1, o2), dim=1)
        offset += offset_1
