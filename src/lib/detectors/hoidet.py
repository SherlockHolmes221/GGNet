...
from models.decode import hoidet_decode, hoidet_decode_match
...
class HoidetDetector(BaseDetector):
    def __init__(self, opt):
        ...
        elif 'pic' in opt.dataset:
            self.corre_mat = np.load(os.path.join(self.opt.root_path, 'PIC_2.0/annotations/hoi_coor_vo.npy'))
    ...
    def process(self, images, return_time=False):
        ...
        with torch.no_grad():
            ...
            if self.opt.test_mul:
                hm_rel = output['hm_rel_refine1'].sigmoid_() * output['hm_rel_refine'].sigmoid_() * output[
                    'hm_rel'].sigmoid_()
            elif self.opt.test_add:
                hm_rel = (output['hm_rel_refine_1'] * 0.1 + output['hm_rel'] * 0.2 + output['hm_rel_refine']).sigmoid_()
            ...
            if self.opt.hm_rel_dcn25_i_casc_match or self.opt.hm_rel_dcn25_i_match:
                dets_obj, dets_sub, rel = hoidet_decode_match(hm_obj, wh, hm_rel, sub_offset, obj_offset, reg=reg,
