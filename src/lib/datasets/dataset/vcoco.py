...
from utils.image import get_affine_transform, affine_transform, draw_umich_gaussian_for_neg
...
class VCOCO(Dataset):
        ...
        if split == 'train':
            ...
            if self.opt.hard_negative:
                self.obj_to_verb = np.load(
                    os.path.join(opt.root_path, "verbcoco/annotations/corre_vcoco_1.npy")).astype(
                    np.float32)
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.opt.soft_gaussian:
            hm_rel_hard = np.zeros((self.num_classes_verb, output_h, output_w), dtype=np.float32)
        ...
        if self.opt.hm_rel_dcn25_i_casc_match or self.opt.hm_rel_dcn25_i_match:
            sub_offset = np.zeros((self.max_rels, 2 * self.num_classes_verb), dtype=np.float32)
            obj_offset = np.zeros((self.max_rels, 2 * self.num_classes_verb), dtype=np.float32)
            offset_mask_offset = np.zeros((self.max_rels, 2 * self.num_classes_verb), dtype=np.uint8)
        else:
        ...
        for k in range(num_rels):
            ...
            if self.opt.soft_gaussian:
                draw_gaussian(hm_rel_hard[hoi_cate], rel_ct_int, radius, factor=8)

            if self.opt.hard_negative:
                obj_cat_index = int(self.cat_ids[anns[hoi['object_id']]['category_id']])
                assert 0 <= obj_cat_index < 80
                cates = np.where(self.obj_to_verb[obj_cat_index] > 0)[0]
                assert hoi_cate in cates
                for i in cates:
                    if i == hoi_cate:
                        continue
                    draw_umich_gaussian_for_neg(hm_rel[i], rel_ct_int, radius=self.opt.hard_negative_radius)
                    if self.opt.soft_gaussian:
                        draw_umich_gaussian_for_neg(hm_rel_hard[i], rel_ct_int, radius=self.opt.hard_negative_radius,
                                                    factor=2)
            ...
            if self.opt.hm_rel_dcn25_i_casc_match or self.opt.hm_rel_dcn25_i_match:
                sub_offset[k][hoi_cate * 2: hoi_cate * 2 + 2] = 1. * rel_sub_offset[0], 1. * rel_sub_offset[1]
                obj_offset[k][hoi_cate * 2: hoi_cate * 2 + 2] = 1. * rel_obj_offset[0], 1. * rel_obj_offset[1]
                offset_mask_offset[k][hoi_cate * 2: hoi_cate * 2 + 2] = 1, 1
            else:
        ...
        if self.opt.reg_offset:
            ret.update({'reg': reg})
        if self.opt.soft_gaussian:
            ret.update({'hm_rel_hard': hm_rel_hard})
        if self.opt.hm_rel_dcn25_i_casc_match or self.opt.hm_rel_dcn25_i_match:
            ret.update({'offset_mask_offset': offset_mask_offset})
        return ret
